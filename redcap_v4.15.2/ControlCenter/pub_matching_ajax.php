...
if ($_GET['action'] == 'getSetupDisplay') {
	...
	$inputs = array(
			...
			array('label' => RCL::redcapURL() . RCView::div(array('class'=>'reqlbl'), RCL::required()),
				'info' => RCL::pubMatchingURLHelp(),
				'input' => RCView::text(array('id' => "pubURL",
								'class' => "x-form-text x-form-field",
								'style' => 'width: ' . getTextWidth(36) . 'px;',
								'value' => empty($redcap_base_url) ? APP_PATH_WEBROOT_FULL : $redcap_base_url))),
      ...
      array('label' => RCL::pubMatchingEmailText() . RCView::div(array('class'=>'reqlbl'), RCL::required()),
        ...
        'input' => RCView::textarea(array('id' => 'pubEmailText', 'class' => 'x-form-field notesbox', 'style' => 'width: 250px; height: 100px;'),
          RCView::escape(empty($pub_matching_email_text) ? RCL::pubEmailIntro() : $pub_matching_email_text))),
			array('label' => RCL::pubInstLabel() . RCView::div(array('class'=>'reqlbl'), RCL::required()),
...
elseif ($_POST['action'] == 'savePubSettings') {
	...
	$pubURL = empty($_POST['pub_url']) ? APP_PATH_WEBROOT_FULL : trim($_POST['pub_url']);
	if (substr($pubURL, -1) !== '/') $pubURL .= '/'; // always add a trailing slash for consistency
  ...
  $pubEmailText = empty($_POST['pub_email_text']) ? '' : trim($_POST['pub_email_text']);
	...
	foreach ($fieldToVal as $field => $val) {
		...
		if (count($sqlArr) === 1) {
			if (mysql_affected_rows()) RCLog::sysConfig($sqlArr[0]);
			$successes++;
		}
	}
...
elseif ($_GET['action'] == 'getMatchExport') {
	...
	$date = date('Ymd');
	RCView::exportFile("redcap_pubs_$date.csv", $content, 'test/csv');
}
