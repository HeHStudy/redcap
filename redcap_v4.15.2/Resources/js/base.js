...
function SortTable(tableid, sortColumn, type)
{
    ...
    else
    {
        ...
        lastSort = sortColumn;
        // what type of values exist in the column? string, int, date?
        switch(type)
        {
            // sort by number
            case 'int':
...
{
    var aVal = a.getElementsByTagName('td')[lastSort].innerHTML.replace(/(<([^>]+)>)/ig,"");
    var bVal = b.getElementsByTagName('td')[lastSort].innerHTML.replace(/(<([^>]+)>)/ig,"");
    var rVal;
    if(aVal == bVal)
    {
        rVal = 0;
	...
	if (regexVal != null)
	{
		...
		if (regexVal2.test(ob.value)) 
		{
			...
			if (min != '' || max != '')
			{
				...
				if (texttype=="integer" || texttype=="number") 
				{
					holder1 = holder1*1;
...
function doFocusRadio(field,form) {
	eval("try{ document."+form+"."+field+"___radio[0].focus() } catch(err) { document."+form+"."+field+".focus() }");
}
...
function chk_cont(pass) {
   ...
   re3 = /[A-Z]/;
   re4 = /^\w+$/;
   return re1.test(pass.value) && re2.test(pass.value) && re3.test(pass.value) && re4.test(pass.value);
}
...
function deleteDocument(doc_id,this_field,id,event_id,delete_page) {
	...
	$.get(delete_page, { s: getParameterByName('s'), id: doc_id, field_name: this_field, record: id, event_id: event_id },function(data) {
		...
		alert('The document "'+$("#"+this_field+"-link").html()+'" has been deleted.');
	});
	// Trigger branching logic in case a "file" field is involved in branching
	doBranching();
	return true;
   ...
   if (chkLen) {
      for (var x = 0; x < chkLen; x++) {
         if (flag == 1) {
			...
			eval("document."+formname+".__chk__"+field.substring(8)+"_"+this_code+".value='';");				
         }
      }
   } else {
      if (flag == 1) {
...
//Data Access Groups page functions
function del_msg(this_group_id,this_group_name,file,app_name) {
	if (confirm('Are you sure you wish to delete the group \"'+this_group_name+'\" ?')) {
		doAjaxGet(file,'pid='+pid+'&action=delete&item='+this_group_id,'group_table');
		setTimeout(function(){editbox_init();},500);
		setTimeout(function(){editbox_init();},1000);
		setTimeout(function(){editbox_init();},2500);
	}
}
function add_group(file,app_name) {
	if (document.getElementById('new_group').value != 'Type new group name') {
		document.getElementById('new_group').disabled=true;
		document.getElementById('new_group_button').disabled=true;
		document.getElementById('progress_img').style.visibility='visible';
		doAjaxGet(file,'pid='+pid+'&action=add&item='+document.getElementById('new_group').value,'group_table');
		setTimeout(function(){editbox_init();},500);
		setTimeout(function(){editbox_init();},1000);
		setTimeout(function(){editbox_init();},2500);
	}
}
function add_user(file,app_name) {
	document.getElementById('groups').disabled=true;
	document.getElementById('group_users').disabled=true;
	document.getElementById('progress_img_user').style.visibility='visible';
	doAjaxGet(file,'pid='+pid+'&action=add_user&user='+document.getElementById('group_users').value+'&group_id='+document.getElementById('groups').value,'group_table');
	setTimeout(function(){editbox_init();},500);
	setTimeout(function(){editbox_init();},1000);
	setTimeout(function(){editbox_init();},2500);
}
function select_group(user,file,app_name) {
	var rnd982g = Math.random();
	req.open('GET',file+'?pid='+pid+'&action=select_group&user='+user+'&rnd982g='+rnd982g);
	req.onreadystatechange = function() {
		if (req.readyState == 4 && req.status == 200) {
			if (req.responseText != '')
				document.getElementById('groups').value = req.responseText;
			else
	...
	$.get(app_path_webroot+'Graphical/highlowmiss.php', { pid: pid, field: field, svc: svc, group_id: group_id }, function(data) {
		...
		if (case_id[0] == 0) {
			...
			for (var i = 1; i <= case_id[0]; i++) {
				...
				html += '<a target="_blank" style="text-decoration:underline;" onclick="$(\'#'+field+'-mperc\').html(\'\'); return true;" href="'+app_path_webroot+'DataEntry/index.php?pid='+pid+'&page='+formVar+'&event_id='+evtid+'&id='+id+'&fldfocus='+field+'#'+field+'-tr" title="'+label+'">'+val+'</a>, ';
			}
...
function highlightTableRow(rowid,event_time) {
	...
	for (var j=0; j<drow.cells.length; j++) {
		$(drow.cells[j]).effect('highlight',{},event_time);
	}
}
//Highlight a table row (by object) for a specified amount of time
function highlightTableRowOb(drow,event_time) {
	for (var j=0; j<drow.cells.length; j++) {
		$(drow.cells[j]).effect('highlight',{},event_time);
	}
}
...
function showProgress(show,ms) {
	...
	if (ms == null) ms = 500;
	// if (!$("#working").length) 	$('body').append('<div id="working" class="white_content_overlay" style="z-index:9999;display:none;position:absolute;padding-right:18px;text-align:center;border:2px solid #aaa;top:40%;left:40%;width:200px;font-size:20px;font-weight:bold;color:#666;"><img src="'+app_path_images+'progress_circle.gif">&nbsp; Working...</div>');
	// if (!$("#fade").length) 	initDialog("fade");
	if (show) {
		centerProgressDiv();
		$("#working").fadeIn(ms);
...
function offsetRangeCheck(event_id,min,max,origDate,checkRange) {
	...
	var newDateVal = $('#date_'+event_id).val();
	if (newDateVal.length < 1) {
		$('#weekday_'+event_id).html('');
		return;
	}
	myDays = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
	myDate=new Date(eval('"'+newDateVal+'"'));
	...
	if (checkRange) {
		...
		var dat1 = new Date(parseInt(dt1[2],10),parseInt(dt1[0],10)-1,parseInt(dt1[1],10)).valueOf();
		var dat2 = myDate.valueOf();
		var daydiff = (dat2-dat1)/(1000*86400);
		var minOrigDate = new Date(parseInt(dt1[2],10),parseInt(dt1[0],10)-1,parseInt(dt1[1],10)-min);
		minOrigDate = (minOrigDate.getMonth()+1)+"/"+minOrigDate.getDate()+"/"+minOrigDate.getFullYear();
		var maxOrigDate = new Date(parseInt(dt1[2],10),parseInt(dt1[0],10)-1,parseInt(dt1[1],10)+max);
		maxOrigDate = (maxOrigDate.getMonth()+1)+"/"+maxOrigDate.getDate()+"/"+maxOrigDate.getFullYear();
		var minmaxtxt = "";
		var alertText = "";
		...
		if (daydiff < (-1*min) && min > 0) {
			...
			alertText = "The date selected ("+newDateVal+") is below the suggested Offset Range Minimum of "+min+" days from the originally generated date ("+origDate+"). You may wish to modify this new value so that it is within the suggested range ("+minmaxtxt+").";
		} else if (daydiff > max && max > 0) {
			//Greater than max
			document.getElementById('rangetext_'+event_id).style.color = 'red';
			document.getElementById('rangetext_'+event_id).style.fontWeight = 'bold';
			minmaxtxt = (min > 0) ? (minOrigDate+" - "+maxOrigDate) : ("maximum "+maxOrigDate);
			alertText = "The date selected ("+newDateVal+") is above the suggested Offset Range Maximum of "+max+" days from the originally generated date ("+origDate+"). You may wish to modify this new value so that it is within the suggested range ("+minmaxtxt+").";
		} else {
			//Not out of range
		...
		if (alertText != "") {
			$("#alert_text").html(alertText);
			$("#alert_text").dialog({ bgiframe: true, modal: true, width: 450, buttons: { Ok: function() {$(this).dialog('close');highlightTableRow("row_"+event_id,2000);} } });
		}
		//Highlight row to emphasize changes
		//highlightTableRow("row_"+event_id,2000);
	}
}

//Run when click Generate Schedule button
function generateSched(record_label) {
	...
	if (document.getElementById('arm').value.length > 0 && (document.getElementById('idnumber').value.length > 0 || document.getElementById('idnumber2').value.length > 0) && document.getElementById('startdate').value.length > 0) {
		...
		alert('Please provide a '+record_label+', a Start Date, and (if applicable) an Arm');
	}
}

//Load table to View/Edit Existing Schedule
function viewSched(record,arm) {
	document.getElementById('table').innerHTML = "<img src='"+app_path_images+"progress_small.gif' class='imgfix'> <span style='color:#555'>Loading...</span>";
	$.get(app_path_webroot+'Calendar/scheduling_ajax.php', { pid: pid, action: 'edit_sched', record: record, arm: arm },
		function(data) {
			//document.getElementById('table').style.display = 'none';
			$('#table').html(data);
			//$('#table').show('blind',{},500);
	...
	if (confirm('Are you sure you wish to delete this calendar event?')) {
		...
		$.get(app_path_webroot+'Calendar/scheduling_ajax.php', { pid: pid, action: 'del_single', record: record, arm: arm, cal_id: cal_id },
			function(data) {
				document.getElementById('table').innerHTML = data;
				if ($('.cal2').length) $('.cal2').datepicker({yearRange: '-100:+10', changeMonth: true, changeYear: true, dateFormat: 'mm/dd/yy'});
				alert('The calendar event has been deleted.');
			}
		);
	}
}
...
function saveEditCalEv(cal_id,record,arm) {
	...
	if ($('#date_'+cal_id).val() != $('#origdate_'+cal_id).val()) {
		...
		var dt1 = $('#date_'+cal_id).val().split("/");
		if (dt1[2].length == 2) dt1[2] = "20"+dt1[2]; // If manually changed year to 2-digit, then prepend with 20.
		var dat1 = new Date(parseInt(dt1[2],10),parseInt(dt1[0],10)-1,parseInt(dt1[1],10)).valueOf();
		var dt2 = $('#origdate_'+cal_id).val().split("/");
		var dat2 = new Date(parseInt(dt2[2],10),parseInt(dt2[0],10)-1,parseInt(dt2[1],10)).valueOf();
	...
	for (var i=1; i<rows.length; i++) {
		...
		if ($('#row_'+this_ev_id).css('display') != 'none') {
			...
			if ($('#date_'+this_ev_id).val().length < 1) {
				alert('Please enter a date for each event');
				return;
			}
		}
	}
	if (all_ev_ids == '') {
		alert('ERROR!\\n\\nYou cannot create a schedule with no dates.');
		return;
	}
...
function view_user(username) {
	...
	document.getElementById('select_username').disabled = true;
...
// Set the value for if a user can create or copy projects (User Controls page)
function set_allow_create_db(username) {
	var val_allow_create_db = ($('#allow_create_db').prop('checked')) ? 1 : 0;
	$.get('user_controls_ajax.php', { action: 'allow_create_db', allow_create_db: val_allow_create_db, username: username}, 
		function(data) {
			if (data == '1') {						
				$('#progress_allow_create_db2').css('visibility','visible');
				setTimeout(function(){
					$('#progress_allow_create_db2').css('visibility','hidden');		
				},2500);
			}
		}
	);
...
// Get hostname (i.e. server_name) and save as cookie
function getServerName() {
	// Set cookie for server name, if doesn't exist
	var server_name = getCookie('server_name');
	if (server_name == "" || server_name != window.location.hostname) {
		setCookie('server_name', window.location.hostname);
		server_name = window.location.hostname;
	}
	// Set cookie for port too (if exists in URL)
	var server_port = window.location.port;
	var server_port_cookie = getCookie('server_port');
	if (server_port != "" && (server_port_cookie == "" || server_port_cookie != server_port) && isNumeric(server_port)) {
		setCookie('server_port',server_port); // Update cookie with current port
	} else if (server_port == "" && server_port_cookie != "") {
		deleteCookie('server_port'); // If we have cookie but now don't need it
	}
	return server_name;
}
...
function dataEntrySubmit(ob) 
{
	...
	if ($('#submit-action').val() != "-- Cancel --") 
	{
		...
		if (require_change_reason && record_exists) 
		{
...
// Initialization functions for global-level pages
function initHomePage() {
	/* 
	// Set Google Translate widget position and display it
	window.onresize = setGoogTransPosition;
	// ONLY if IE (because we're hardcoding it for IE because it doesn't work), set Google Translate widget position and display it
	if (isIE) {
		setGoogTransPosition();
	}
	 */
}
...
function initPage() {
	// Resize windows
	resizeMainWindow();
	// Adjust page dimensions and settings when window is resized
	// window.onresize = function() {
		// Adjust page dimensions whenever window is resized by user
		// resizeMainWindow();
		// Set Google Translate widget position and display it
		// setGoogTransPosition();		
	// }
	...
	// For iOS, if user tries one-finger scroll, give notice to do two-finger swipe
	if (isIOS) {
		// Show notice msg if try to use one-finger scroll (but only if 'west' or 'center' divs have scrollbars)
		if (IsYScrollBarExist('center') || IsYScrollBarExist('west')) {
			$(window).scroll(function(){
				if ($('#iOS_scroll_msg').css('display') == 'none') {
					$('#iOS_scroll_msg').css('display','block');
					setTimeout(function(){ $('#iOS_scroll_msg').toggle('blind','fast'); },7000);
...
function initAutoLogout(resettime,logouttime) {
	...
	if ($('#redcap_login_a38us_09i85').length) return false;
	...
	var warn_timeout3 = "<b>Due to inactivity, your REDCap session has expired.</b><br>Click the button below to log in again.";
...
function autoLogoutDialog(msg,doLogout,resettime,logouttime) {
	...
	// Display dialog
	$('#'+div_id).dialog({ bgiframe: true, modal: true, width: 470, title: 'REDCap Auto Logout Warning', 
...
function openUserHistoryList() {
	...
	$('#userList').dialog('destroy');
	$('#userList').dialog({ title: title, bgiframe: true, modal: true, position: ['center',50], width: 1070, buttons: { Close: function() { $(this).dialog('close'); } } });
...
// Retrieve the class or id of the Google Translation widget drop-down
function getGoogTransWidgetId() {
	if (!$('#google_translate_element').length) return '';
	var translate_element_id = '';
	$('#google_translate_element select').each(function() {
		translate_element_id = ($(this).prop('id') != null && $(this).prop('id') != "") ? '#'+$(this).prop('id') : '.'+$(this).prop('class');
	});
	return translate_element_id;
}

// If a string has a hash #, remove hash and all text that follows
function removeHash(hash) {
	var pos = hash.indexOf(googleTransAnchor);
	return (pos > -1) ? hash.substring(0,pos) : hash;	
}
// If a string has a hash #, obtain hash and all text that follows
function getHash(href) {
	var pos = href.indexOf(googleTransAnchor);
	return  (pos > -1) ? href.substring(pos) : '';
}
// Return hash string from URL
function addGoogTrans() {
	return ''; // NOT USING YET
	// Check if Google Translation is enabled
	if (typeof(googleTransAnchor) == "undefined") return '';
	// First, check the translate widget selection
	hash = $(getGoogTransWidgetId()).val();
	if (hash != '') return googleTransAnchor + hash;
	// Now heck URL
...
// Append hashes to form menu links and form actions
function appendGoogTransHash(hash) {
	if (hash == '') return;
	// Append hash to form links on left-hand menu
	$('#west div.notranslate a').each(function() {
		var this_href = $(this).prop('href');
		this_href = removeHash(this_href);
		this_href += hash;
		$(this).prop('href', this_href);
	});
	// Append hash to 'action' of all forms
	$('form').each(function() {
		var this_action = $(this).prop('action');
		this_action = removeHash(this_action);
		this_action += hash;
		$(this).prop('action', this_action);
	});
	// Hide footer div because of css issues
	$('#south').css('display','none');
}
// Create Google Translate widget and run initialization for REDCap
function googleTranslateElementInit() {
	// Create widget
	new google.translate.TranslateElement({ pageLanguage: 'en' }, 'google_translate_element');
	// Custom javascript to run after Google Translate widget loads
	$(function() {
		// Weird IE issue that causes page to disappear
		if (isIE) $('body').css('position','');
		// First, add as trigger to widget that will append text to data entry form links and to 'action' of all forms
		$(getGoogTransWidgetId()).change(function() { 
			appendGoogTransHash(googleTransAnchor+$(this).val()); 
			// Check/uncheck default language checkbox (project-level only)
			try { document.getElementById('googDefChk').checked = ($(this).val() == google_translate_default); } catch(e) {}
			// Reveal Google Translate disclaimer (only once per page)
			if ($("#googTransDisclaimer").html().length < 1) {
				$("#googTransDisclaimer").html("<b>DISCLAIMER:</b><br>Translations of pages in REDCap are performed by Google Translate. Google Translate is a free online language translation service that can translate text and web pages into different languages. Computerized translations are only an approximation of the website's original content. The translation should not be considered exact and in some cases may include incorrect or offensive language. REDCap does not warrant the accuracy, reliability, or timeliness of any information translated by this system. No personal information or data from your project(s) will be sent to Google during the translation process.<div style='text-align:center;padding-top:10px;'><input type='button' value='I Understand' style='font-size:11px;' onclick=\"$('#googTransDisclaimer').toggle('blind',{},500);\">");
				$("#googTransDisclaimer").offset({ top: 80, left: ($(window).width() - 50 - $("#googTransDisclaimer").width()) });
				$('#googTransDisclaimer').toggle('blind',{},500);
			}
		});
		// Second, add hashes to links and form action when page loads
		appendGoogTransHash(getHash(window.location.href));			
		// Display option for those with Design/Setup rights to set default language (project-level only)
		try {
			if (typeof(pid) != 'undefined') {
				$('#google_translate_element').append('<div id="googDefDiv"><span id="googDefSave">Saved!</span> Set as default language <input id="googDefChk" type="checkbox" onclick="setDefLang()" class="imgfix2"></div>');
				if (google_translate_default != '' && google_translate_default == getHash(window.location.href).replace(googleTransAnchor,'')) {
					$('#googDefChk').prop('checked',true); // Check default language checkbox
				}
			}
		} catch(e) {}
		// Set Google Translate widget position and display it
...
// Set Google Translation widget position and display it
function setGoogTransPosition() {
	if (!$('#google_translate_element').length) return '';
	if (typeof(pid) != 'undefined') {
		// Project-level
		var top = 0;
		//var left = $(window).width() - $('#google_translate_element').width() - (isIE7 ? 165 : 30); // Strange IE7 issue
		var left = $(window).width() - $('#google_translate_element').width() - 30; 
	} else {
		// Home page
		var top = 30;
		var left = 0;
		$('#google_translate_element').width('98%');
		$('#google_translate_element').css('text-align','right');
	}
	$('#google_translate_element').offset({ top: top, left: left });
	$("#google_translate_element").css('display','block');
	return true;
}

// Set default language for Google Translate
function setDefLang() {
	var is_checked = (document.getElementById('googDefChk').checked) ? 1 : 0;
	var new_lang = $(getGoogTransWidgetId()).val();
	if (new_lang == '') {
		document.getElementById('googDefChk').checked = (!document.getElementById('googDefChk').checked);
		return;
	}
	$.post(app_path_webroot+"ProjectGeneral/set_default_project_language.php?pid="+pid, { is_checked: is_checked, new_lang: new_lang }, function(data) {
		if (data != "0") {
			// Set variable with new value
			google_translate_default = new_lang;
			// Show 'saved' status
			$('#googDefSave').css('visibility','visible');
			setTimeout(function() { $('#googDefSave').css('visibility','hidden'); },1000);
		} else {
			document.getElementById('googDefChk').checked = (!document.getElementById('googDefChk').checked);
			alert("Woops! An error occurred. Please try again");
		}
...
function delete_project(this_pid,ob) {
	$.post(app_path_webroot+'ProjectGeneral/delete_project.php?pid='+this_pid, { action: 'prompt' }, function(data) {
		...
		$('#del_db_dialog').dialog({ bgiframe: true, title: 'Permanently delete this project?', modal: true, width: 550, buttons: { 
			...
			'Delete the project': function() {
				...
				$.post(app_path_webroot+'ProjectGeneral/delete_project.php?pid='+this_pid, { action: 'delete' }, function(data) {
					$(':button:contains("Cancel")').html('Please wait...');
					$(':button:contains("Delete the project")').css('display','none');
					if (data == '1') {
						var msg = "The project and all its data were successfully deleted from REDCap.\n\n";
...
function initWidgets() {
	...
	if ($('.cal').length) $('.cal').datepicker({ 
		onSelect: function(){ $(this).blur(); try{ calculate();doBranching(); }catch(e){ } },
	...
	$('.time2').timepicker({
		onClose: function(){ $('.time2').before().blur(); try{ calculate();doBranching(); }catch(e){ } },
	...
	$('.datetime_ymd').datetimepicker({
		onClose: function(){ $('.datetime_ymd').before().blur(); try{ calculate();doBranching(); }catch(e){ } },
	...
	$('.datetime_mdy').datetimepicker({
		onClose: function(){ $('.datetime_mdy').before().blur(); try{ calculate();doBranching(); }catch(e){ } },
	...
	$('.datetime_dmy').datetimepicker({
		onClose: function(){ $('.datetime_dmy').before().blur(); try{ calculate();doBranching(); }catch(e){ } },
	...
	$('.datetime_seconds_ymd').datetimepicker({
		onClose: function(){ $('.datetime_seconds_ymd').before().blur(); try{ calculate();doBranching(); }catch(e){ } },
		buttonText: 'Click to select a date', yearRange: '-100:+10', changeMonth: true, changeYear: true, dateFormat: 'yy-mm-dd',
		hour: currentTime('h'), minute: currentTime('m'), buttonText: 'Click to select a date/time', 
	...
	$('.datetime_seconds_mdy').datetimepicker({
		onClose: function(){ $('.datetime_seconds_mdy').before().blur(); try{ calculate();doBranching(); }catch(e){ } },
		buttonText: 'Click to select a date', yearRange: '-100:+10', changeMonth: true, changeYear: true, dateFormat: 'mm-dd-yy',
		hour: currentTime('h'), minute: currentTime('m'), buttonText: 'Click to select a date/time', 
		showOn: 'button', buttonImage: app_path_images+'datetime.png', buttonImageOnly: true, timeFormat: 'hh:mm:ss', constrainInput: false
	});
	$('.datetime_seconds_dmy').datetimepicker({
		onClose: function(){ $('.datetime_seconds_dmy').before().blur(); try{ calculate();doBranching(); }catch(e){ } },
		buttonText: 'Click to select a date', yearRange: '-100:+10', changeMonth: true, changeYear: true, dateFormat: 'dd-mm-yy',
		hour: currentTime('h'), minute: currentTime('m'), buttonText: 'Click to select a date/time', 
		showOn: 'button', buttonImage: app_path_images+'datetime.png', buttonImageOnly: true, timeFormat: 'hh:mm:ss', constrainInput: false
	});
			...
			$.get(app_path_webroot+'Design/delete_form.php', { pid: pid, form_name: form_to_delete },
				function(data) {
					if (data=='1' || data=='2') {
						...
						//Give confirmation
						//showProgress(0);
						alert("The data collection instrument and all its fields have been successfully deleted!");
						if (data == '2') update_pk_msg(true,'form');
					} else if (data == '0') {
						alert(woops);
					} else if (data == '3') {
						alert("The data collection instrument could NOT be deleted because some of the fields that exist on that instrument are being used by the randomization module.");
					}
				}
			);
		}
...
function update_pk_msg(reload_page,moved_source) {
	$.get(app_path_webroot+'Design/update_pk_popup.php', { pid: pid, moved_source: moved_source }, function(data) {
		if (data != '') { // Don't show dialog if no callback html (i.e. no records exist)
			...
			$('#update_pk_popup').dialog({title: 'NOTICE: Record identifer field was changed!', bgiframe: true, modal: true, width: 600, buttons: {
				'Okay': function() { 
					$(this).dialog('close'); 
					if (reload_page != null) {
...
// Prints the page (also makes project-level pages more viewable for printing)
function printpage(obj) {
	// Hide West and South divs for project-level pages and add a Reset button to reload page again
	if ($('#center').length) {
		// Add reset button and hide original button/link
		$(obj).after('&nbsp;<a style="text-decoration:underline;" href="javascript:;" onclick="window.location.href=window.location;return false;">Reset page display</a>');
		$(obj).hide();
		// Set div content as entire page content
...
function fitDialog(ob) {
	...
	if (($(ob).height()+110) >= winh) {
		$(ob).css("overflow","auto");
		$(ob).css("height",(winh-110)+"px");
	}
}
	...
	$('#stopActionPrompt').dialog({ bgiframe: true, modal: true, width: (isMobileDevice ? $('body').width() : 550), 
		close: function(){
			...
			if (obname.substring(0,8) == '__chkn__'){
				...
				$('#form input[name="'+obname.replace('__chkn__','__chk__')+'_'+ob.attr('code')+'"]').val('');
			...
			setTimeout(function(){
				$('#stopActionReturn').dialog({ bgiframe: true, modal: true, width: 320, buttons: { 
					'Continue survey': function() { highlightTableRow(varname+'-tr',2500); $(this).dialog('close'); }
				} });
			},100);
		},
		buttons: { 
			'Continue survey and undo last response': function() { $(this).dialog('close'); },
...
// Direct to Shared Library upload page
function shareInstr() {
...
// Display prompt for installing Google Chrome Frame for IE
function displayChromeFrameInstallPopup() {
	var div_id = 'google_chrome_frame_install_prompt';
	$('body').append('<div id="'+div_id+'" style="display:none;"></div>'); 
	CFInstall.check({ mode: 'inline', node: div_id });
}
...
function renderCharts(nextfields,charttype,results_code_hash) {
	...
	if (hash != '') {
		...
		url += '&s='+hash+'&__results='+getParameterByName('__results');
	} else if (record != '') {
		...
		$('#randomizeDialog').dialog({ bgiframe: true, modal: true, width: 750, open: function(){fitDialog(this)}, 
			...
			buttons: {
				...
				'Randomize': function() {
					...
					$.post(app_path_webroot+'Randomization/randomize_record.php?pid='+pid, { redcap_data_access_group: $('#random_form select[name="redcap_data_access_group"]').val(), existing_record: document.form.hidden_edit_flag.value, action: 'randomize', record: record, fields: critFlds.join(','), field_values: critFldVals.join(',') }, function(data){
						if (data == '0') {
							alert(woops);
							return;
						}
...
// Obtain server name using stored cookie, if possible (used later by PHP)
