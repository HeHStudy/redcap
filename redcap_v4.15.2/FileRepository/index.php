...
if (isset($_GET['id']) && $_GET['id']) {
	$elements1[]=array('rr_type'=>'textarea', 'name'=>'docs_comment', 'label'=>$lang['docs_23'].' &nbsp;', 'style'=>'width:250px;height:70px;font-family:Arial;font-size:12px;');
	...
	switch ($_POST['submit'])
	{
		case 'Upload File':
			...
			if (strlen($dummy['tmp_name']) > 0 && empty($errors)) 
			{
				...
				if ($edoc_storage_option) 
				{
					$file_hash = md5($dummy_file_content);
					$target_path = $webdav_path . $stored_name;
					$wdc->put($target_path,$dummy_file_content);
					$wdc->get($target_path,$buffer);
					
					if ($file_hash != md5($buffer)) {
						$errors[] = "File validation error on ".$dummy_file_name.".";
					} else {
						$upload_success = TRUE;
					}	
				} 
				...
				{
					if (move_uploaded_file($dummy_tmp_name, EDOC_PATH . $stored_name)) 
					{
						$buffer = fread(fopen(EDOC_PATH . $stored_name, "rb"), filesize(EDOC_PATH . $stored_name));
						$file_hash = md5($dummy_file_content);
						if ($file_hash != md5($buffer)) {
							$errors[] = "File validation error on ".$dummy_file_name.".";
						} else {
					...
					if (mysql_query($sql)) {
						...
						if (mysql_query($sql)) {
							...
							if (mysql_query($sql)) {
								...
								mysql_query("DELETE FROM redcap_edocs_metadata WHERE doc_id='".$doc_id."';");
				...
				if ($database_success === FALSE) {
					$context_msg = "{$lang['global_01']}: {$lang['docs_47']}";
					if ($super_user) {
						$context_msg .= $lang['docs_48'];
					}
				}
			}
			break;		
		...
		case 'Delete File':
			...
			if ($result) {
				...
				mysql_query("DELETE FROM redcap_docs WHERE docs_id='".$data->docs_id."' AND project_id='".$project_id."';");
				if ($data->doc_id != NULL) {
					mysql_query("DELETE FROM redcap_edocs_metadata WHERE doc_id='".$data->doc_id."';");
					mysql_query("DELETE FROM redcap_docs_to_edocs WHERE docs_id='".$data->docs_id."' AND doc_id='".$data->doc_id."';");
...
{
	$dag_groups = mysql_result(mysql_query("select count(1) from redcap_data_access_groups where project_id = $project_id"), 0);
	if ($dag_groups > 0) {
...
{	
	...
	else
	{
		...
		$q = mysql_query($sql);
		foreach (mysql_fetch_assoc($q) as $field=>$value) {
			if ($field == "docs_date") {
...
if (!isset($_GET['id'])) {
	...
	elseif ($_GET['type'] == 'export' && $user_rights['data_export_tool'] > 0) 
	{
		...
		if ($user_rights['group_id'] != "") {
			...
			$group_sql = "AND (";
			$q = mysql_query("select username from redcap_user_rights where project_id = $project_id and 
		...
		$qrs_pubs = mysql_query($rs_pubs_sql);
		
		if (mysql_num_rows($qrs_pubs) < 1) {
			...
			$page_instructions .=  "<option class='notranslate' value='excel'"; if ($_GET['export'] == 'excel') $page_instructions .= " selected";
			$page_instructions .=  ">Microsoft Excel (CSV)</option>";
			$page_instructions .=  "<option class='notranslate' value='r'"; if ($_GET['export'] == 'r') $page_instructions .= " selected";
			$page_instructions .=  ">R</option>";
			$page_instructions .=  "<option class='notranslate' value='sas'"; if ($_GET['export'] == 'sas') $page_instructions .= " selected";
			$page_instructions .=  ">SAS</option>";
			$page_instructions .=  "<option class='notranslate' value='spss'"; if ($_GET['export'] == 'spss') $page_instructions .= " selected";
			$page_instructions .=  ">SPSS</option>";
			$page_instructions .=  "<option class='notranslate' value='stata'"; if ($_GET['export'] == 'stata') $page_instructions .= " selected";
			...
			if (!isset($_GET['export']) || $_GET['export'] == '') {
				...
				$num_total_files = mysql_result(mysql_query($sql),0);
			}
			...
			$old_docs_ts = "";
			$i = 0;
			while ($rs_pubs = mysql_fetch_assoc($qrs_pubs)) 
			{				
				$i++;
				...
				$docs_type = strtoupper(substr($docs_comment,0,strpos($docs_comment," ")));
				...
				if ($docs_type == "DATA") {
					...
					$sql = "select * from redcap_docs where project_id = $project_id and docs_id = " . ($docs_id+2) . " 
							and docs_comment = '".prep($rs_pubs['docs_comment'])."' and docs_date = '".prep($rs_pubs['docs_date'])."'
							and export_file = 1	and (docs_name like '%\_DATA\_LABELS\_2%.csv' $include_legacy_csv_label) limit 1";
					$q = mysql_query($sql);
					$show_csv_labels_icon = (mysql_num_rows($q) > 0) ? "visible" : "hidden";
					// display row
					$page_instructions .=  "</td>
											<td valign='top' style='text-align:right;width:100px;padding-top:10px;'>
												<a href='" . APP_PATH_WEBROOT . "FileRepository/file_download.php?pid=".$project_id."&id=".($docs_id+2)."' style='visibility:$show_csv_labels_icon;text-decoration:none;'>
			...
			$page_instructions .=  "<form method='post' action='".PAGE_FULL."?pid=$project_id'>
									...
									<tr class='grp2'>
										<td style='font-family:Verdana;font-size:12px;text-align:right;font-weight:normal;'>
											{$lang['docs_43']}
											<select class='notranslate' name='filetypes' onchange='location.href=\"".$_SERVER['PHP_SELF']."?pid=$project_id&filetype=\"+ this.value+addGoogTrans();'>
											<option value=''>{$lang['docs_44']}</option>";
											
			//Show dropdown to filter file types								
			$q = mysql_query("select * from redcap_docs WHERE project_id = $project_id  AND export_file = 0 ORDER BY docs_id DESC");
